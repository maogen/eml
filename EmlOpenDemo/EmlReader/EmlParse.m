...
-(EmlItem *)getItemWithPath:(NSString *)filePath {
    ...
    if (exists) {
        [self itemInit];
        [self divideEML:[[NSString alloc] initWithContentsOfFile:filePath encoding:encUTF8
                                                           error:nil]];
        [self divideEML:[[NSString alloc] initWithContentsOfFile:filePath encoding:encgb2312 error:nil]];
        return item;
    }
    else
        return nil;
...
- (EmlItem *)getItem:(NSData *)data{
    [self itemInit];
    [self divideEML:[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]];
    [self divideEML:[[NSString alloc] initWithData:data encoding:encgb2312]];
    return item;
